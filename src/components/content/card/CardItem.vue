<template>
  <div
    class="cardItem"
    @click.stop="deviceState!='phone'||deviceState=='mobile'? checkCard(cardItem._key, cardTargetIndex, cardItem.groupKey):null"
    :style="{background:cardItem.finishPercent==2?'#E3E3E3':null,borderLeft:'3px solid '+
              color[
  ...
  watch: {
    cardMemberList: {
      handler(newVal, oldVal) {
        if (oldVal != newVal && newVal.length > 0 && this.linkState == 1) {
          //他人界面里共同群@他的
          //卡片的群key判断权限
    ...
    //           if (
    //             item._key == localStorage.getItem("userKey") &&
    //             this.linkState == 1
    //           ) {
    //             this.changeCardShowState(true);
    //             this.changeCardDetailType(2);
