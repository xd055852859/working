  ...
  <div class="phoneTask" @click="addLabelState=false">
    ...
    <div class="phoneTask-container">
      <!-- <div class="phoneTask-item">
        ...
        <div
          ...
          @click.prevent="checkCard(item._key,item,item.groupKey)"
        >
      ...
      <div class="phoneTask-item" v-for="(item,index) in taskInfo" :key="'taskInfo'+index">
        ...
        <div
          ...
          @click.stop="checkCard(value._key,value,value.groupKey)"
        >
          <van-swipe-cell :style="{width:'100%',height:'100%'}">
            ...
            <template #right>
              <van-button
                ...
                type="danger"
                text="删除"
                @click="deleteCard({cardKey:value._key,groupKey:value.groupKey})"
              />
    ...
    <div v-if="cardShowState" class="phoneTaskItem-cardInfo">
      ...
      <CardInfo
        ...
        :style="deviceState=='phone'||deviceState=='mobile'?{width:(deviceWidth- 60)+'px',height:'100%'}:null"
      />
    </div>
    ...
    <!-- <div class="phoneChat-close" @click="closePhoneChat" v-show="iframeShowState">
...
import Message from "../leftSide/message/Message";
...
export default {
  ...
  methods: {
    ...
    getName(groupType) {
      if (groupType == 1) {
        this.groupAvatar =
          this.user && this.user.profile && this.user.profile.avatar
  ...
  watch: {
    ...
    groupKey(newVal, oldVal) {
      if (oldVal !== newVal && newVal) {
        ...
        if (this.groupType == 3) {
          this.getTaskList({
            typeBoard1: this.groupType==1?3:this.groupType,
            targetUGKey: newVal,
            finishPercentArray: "[0, 1,10]",
            fileDay: 14,
          });
        }
      }
